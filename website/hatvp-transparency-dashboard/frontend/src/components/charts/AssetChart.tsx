...
export default function AssetChart({ items, emptyLabel, language, compact = false }: { items: BreakdownItem[]; emptyLabel: string; language: Language; compact?: boolean }) {
  ...
  return (
    <div className="w-full min-w-0" role="img" aria-label={emptyLabel}>
      ...
      <div className={`${compact ? "mt-4 pt-4" : "mt-6 pt-5"} grid gap-x-6 gap-y-2 border-t border-slate-100 sm:grid-cols-2`} aria-label="Asset chart values">{chartData.map((item) => <div key={item.label} className="flex min-w-0 items-center justify-between gap-3 text-sm"><span className="min-w-0 truncate font-semibold text-slate-700">{item.displayLabel}</span><span className="shrink-0 font-bold text-ink">{formatCurrency(item.value, language)}</span></div>)}</div>
    </div>
  );
}
