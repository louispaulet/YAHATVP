...
export default function GenderRatioChart({ items, unknownRows, emptyLabel, language, chartLabel, legendLabel, maleLabel, femaleLabel, unknownNote, compact = false }: Props & { compact?: boolean }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col items-center gap-5 sm:flex-row">
        <div className={`${compact ? "h-40 sm:h-44" : "h-56 sm:h-60"} w-full min-w-0`} role="img" aria-label={`${chartLabel}: ${description}`}>
          <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data} dataKey="rows" nameKey="displayLabel" innerRadius={compact ? 45 : 62} outerRadius={compact ? 68 : 92} paddingAngle={3}>{data.map((item, index) => <Cell key={item.label} fill={colors[index]} />)}</Pie><Tooltip formatter={(value) => formatNumber(Number(value), language)} /></PieChart></ResponsiveContainer>
        </div>
