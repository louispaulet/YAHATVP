...
function parentClass(active: boolean): string {
  return active
    ? "relative rounded-xl bg-white/70 px-3 py-2 text-[13px] font-bold text-ink transition after:absolute after:inset-x-3 after:-bottom-1 after:h-0.5 after:rounded-full after:bg-emerald hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-emerald"
    : "relative rounded-xl px-3 py-2 text-[13px] font-semibold text-slate-500 transition hover:bg-white/50 hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-emerald";
}

function childClass({ isActive }: { isActive: boolean }): string {
  return isActive
    ? "relative px-1 py-1.5 text-xs font-bold text-emerald transition after:absolute after:inset-x-1 after:-bottom-0.5 after:h-0.5 after:rounded-full after:bg-emerald focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-emerald"
    : "relative px-1 py-1.5 text-xs font-semibold text-slate-500 transition hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-emerald";
}
      ...
      <footer className="mt-auto border-t border-slate-200/80">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-8 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between lg:px-8">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-2">
            <a className="inline-flex items-center gap-1.5 font-semibold text-slate-700 underline decoration-lime underline-offset-4" href="https://github.com/louispaulet/YAHATVP/tree/main" target="_blank" rel="noreferrer"><GitBranch size={13} strokeWidth={1.8} aria-hidden="true" />{locale.footer.project}</a>
            <a className="inline-flex items-center gap-1.5 font-semibold text-slate-700 underline decoration-lime underline-offset-4" href="https://www.hatvp.fr/" target="_blank" rel="noreferrer">hatvp.fr<ExternalLink size={13} strokeWidth={1.8} aria-hidden="true" /></a>
          </div>
        </div>
      </footer>
    </div>
