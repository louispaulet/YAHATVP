...
function QualityBadge({ status, labels, fallback, linkTo, linkLabel }: { status: string | null; labels: Record<string, string>; fallback: string; linkTo?: string; linkLabel?: string }) {
  const review = status && status !== "valid";
  const label = labels[status || ""] || fallback;
  const className = `inline-flex max-w-full break-words rounded-full px-2.5 py-1 text-center text-[0.7rem] font-bold leading-4 transition hover:brightness-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald ${review ? "bg-amber-100 text-amber-900" : "bg-sky/20 text-ink"}`;
  if (linkTo) return <Link to={linkTo} aria-label={linkLabel} className={className}>{label}</Link>;
