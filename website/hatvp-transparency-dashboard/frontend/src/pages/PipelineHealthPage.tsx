  ...
  return <div className="mx-auto max-w-7xl space-y-8 px-5 py-8 lg:px-8 lg:py-12">
    ...
    {data && <>
      <section className="grid gap-4 md:grid-cols-3">
        <div className="dashboard-card pipeline-health-countdown p-6 text-white md:col-span-2" data-testid="pipeline-countdown-card"><p className="text-xs font-bold uppercase tracking-[0.15em] text-white/70">{locale.pipelineHealth.nextIngestion}</p><p className="mt-3 text-4xl font-black tabular-nums sm:text-5xl" data-testid="pipeline-countdown" role="timer" aria-live="polite">{countdown(data.nextIngestionAt, now, locale.pipelineHealth.countdown)}</p><p className="mt-3 text-sm text-white/80">{new Intl.DateTimeFormat(language === "fr" ? "fr-FR" : "en-GB", { dateStyle: "full", timeStyle: "short" }).format(new Date(data.nextIngestionAt))}</p></div>
