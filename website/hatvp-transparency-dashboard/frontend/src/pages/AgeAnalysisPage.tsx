...
export function AgeAnalysisPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">
      <NavLink to="/search" className="mb-6 inline-flex min-h-10 items-center gap-2 text-sm font-bold text-emerald transition hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-emerald"><ArrowLeft size={16} strokeWidth={2} aria-hidden="true" />{locale.nav.search}</NavLink>
      ...
      <section className="dashboard-card relative z-10 mt-4 p-5 sm:p-6"><form onSubmit={submit}><label htmlFor="age-analysis-search" className="text-sm font-bold text-ink">{labels.inputLabel}</label><div className="mt-3 flex flex-col gap-3 sm:flex-row"><div className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 focus-within:border-emerald focus-within:ring-2 focus-within:ring-emerald/30"><Search aria-hidden="true" className="size-5 shrink-0 text-emerald" /><input id="age-analysis-search" value={input} onChange={(event) => setInput(event.target.value)} placeholder={labels.placeholder} maxLength={120} className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-ink outline-none placeholder:font-normal placeholder:text-slate-400" /></div><button type="submit" className="min-h-11 rounded-2xl bg-emerald px-6 py-3 text-sm font-bold text-white transition hover:bg-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald">{labels.submit}</button></div><p className="mt-3 text-xs leading-5 text-slate-500">{labels.hint}</p>{!params.get("q") && <p className="mt-2 text-xs font-semibold text-emerald">{labels.exampleLabel}: {DEFAULT_QUERY}</p>}</form></section>
      ...
      {data && <>
        {data.matches.length > 1 && <section className="mt-8" aria-live="polite"><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald">{labels.matches}</p><div className="mt-3 flex flex-wrap gap-3">{data.matches.map((match) => { const matchName = nameOf(match, locale.search.unknownDeclarant); const selected = matchName === query; return <button type="button" key={match.personKey || match.primaryUuid} aria-pressed={selected} onClick={() => selectMatch(match)} className={`min-h-11 rounded-full border px-4 py-2 text-sm font-bold transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald ${selected ? "border-emerald bg-emerald/10 text-emerald" : "border-slate-200 bg-white text-ink hover:border-emerald hover:text-emerald"}`}>{matchName} <span className="ml-1 text-xs font-normal text-slate-500">· {formatNumber(match.declarationCount, language)} {labels.declarations}</span></button>; })}</div><p className="sr-only">{labels.selectedMatch.replace("{name}", nameOf(data.person, locale.search.unknownDeclarant))}</p></section>}
