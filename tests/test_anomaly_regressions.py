...
def _dob_occurrence() -> dict[str, Any]:
    items = detect_anomalies(layer_tables(), historical_tables())
    return next(item for item in items if item["rule_id"] == "PERSON_DOB_IMPLAUSIBLE")
...
def test_implausible_birth_uses_reference_date_and_max_age() -> None:
    assert implausible_birth("1925-01-01", "2026-01-01")
    assert not implausible_birth("1925-01-01", "2026-01-01", 110)
    assert not implausible_birth("1945-01-01", "2026-01-01")
    assert implausible_birth("2026-01-02", "2026-01-01")


def test_known_occurrence_keeps_original_rule_id_in_registry() -> None:
    current = layer_tables()
    original = _dob_occurrence()
...
def test_regression_occurrence_keeps_original_rule_id_and_metadata() -> None:
    original = _dob_occurrence()
    first = upsert_registry([original], [], "2026-01-01")
    first[0]["status"] = "superseded"
    repeated = detect_anomalies(layer_tables(), historical_tables(), first)
    lifecycle = next(
        item
        for item in repeated
        if item["anomaly_key"] == original["anomaly_key"]
        and item["rule_id"] == "ANOMALY_REGRESSION"
        and item["original_rule_id"] == original["rule_id"]
    )

    updated = upsert_registry([lifecycle], first, "2026-02-01")
    row = updated[0]
    assert row["rule_id"] == "PERSON_DOB_IMPLAUSIBLE"
