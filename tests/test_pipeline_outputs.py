...
def test_changed_xml_hash_triggers_processing(tmp_path: Path, monkeypatch) -> None:
    ...
    row = pl.read_parquet(
        output / "hatvp/silver/declarations/snapshot_date=2026-08-16/data.parquet"
    ).row(0, named=True)
    assert row["source_url"] == "fixture://declarations.xml"
    assert row["source_sha256"] == first["xml_sha256"]
    warning_status(
...
def test_curated_parquet_schema_is_stable_for_empty_and_null_only_rows(tmp_path: Path) -> None:
    ...
    assert empty_schema["source_snapshot_date"] == pl.Date
    assert empty_schema["bronze_record_key"] == pl.String
